...
let init = (conf) => {
	...
	CONSOLE_HEAD = conf['CONSOLE_HEAD'];
}
...
let buildStr = (baseDir) => {
	if (__verbose) console.log(CONSOLE_HEAD, `BuildDirectory: ${baseDir}`)
	/**
	...
	gmComponentsDirs.forEach((moduleDir) => {
		...
		if (__verbose) console.log(CONSOLE_HEAD, 'Load'.yellow, baseDir ? path.relative(baseDir, _p) : _p);
	...
	let resolveKey = (o, k) => {
		...
		if (_.isObject(v) && v.template) { //template
			...
			if (__verbose) console.log(`[GM] Render: ${v.template}`);
