...
let load = (dir, baseDir) => {
	...
	if (!conf.main && !conf.input) return console.error('no main or input gm.json');
	...
	if (!fs.existsSync(templatePath)) return console.error('no file ' + templatePath + ' that defined in gm.json by main');
