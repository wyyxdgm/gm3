...
let load = (dir, baseDir, conf) => {
	...
	if ((!conf || !conf.main) && !gmExists) return console.error('no gm.json or conf'.red);
	...
	if (conf.input && conf.input.template && conf.input.data) {
		...
		if (conf.main.indexOf('.') >= 0) {
			if (conf.main.indexOf('.js') >= 0) {
				[templatePath, jsonPath] = [conf.main, conf.main.substring(0, conf.main.indexOf('.js')) + '.json'].map(_p => path.join(dir, _p));
			} else {
				[templatePath, jsonPath] = [conf.main, conf.main.substring(0, conf.main.indexOf('.'))].map(_p => path.join(dir, _p));
			}
		} else {
	...
	console.log(templatePath, jsonPath)
